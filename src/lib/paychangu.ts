...
export async function triggerPayChanguPayment(
  ...
  return new Promise((resolve, reject) => {
      ...
      try {
        checkoutFn({
          ...
          customization: {
            ...
            description: config.customization?.description || `Subscription Payment for ${config.amount.toLocaleString()} MWK`,
          ...
          callback: (res: any) => {
            const successRes: PayChanguSuccessResponse = {
              status: 'successful',
              tx_ref: txRef,
              transaction_id: res?.transaction_id || `TX-${Math.floor(1000000 + Math.random() * 9000000)}`,
              amount: config.amount,
              currency: config.currency || 'MWK',
              channel: config.channel || 'online_payment',
        ...
        console.warn('Payment popup invocation failed, falling back to direct processing:', err);
      }
    }
    ...
    setTimeout(() => {
      const channelName = 
        config.channel === 'airtel' ? 'Airtel Money' :
        config.channel === 'mpamba' ? 'TNM Mpamba' :
        config.channel === 'card' ? 'Card Payment' :
        'Bank Transfer';

      const successRes: PayChanguSuccessResponse = {
        status: 'successful',
        tx_ref: txRef,
        transaction_id: `TX-${Date.now().toString().slice(-8)}`,
        amount: config.amount,
        currency: config.currency || 'MWK',
        channel: channelName,
