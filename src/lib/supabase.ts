...
export const normalizeProfile = (row: any): UserProfile => {
  ...
  const trialEnds = row.trialEndsAt || row.trial_ends_at || new Date(now.getTime() + 14 * 24 * 60 * 60 * 1000).toISOString();
...
export const ensureProfileExists = async (userId: string, requiredRole?: UserRole): Promise<void> => {
  ...
  try {
    ...
    const now = new Date();
    const trialEndsAt = meta.trialEndsAt || new Date(now.getTime() + 14 * 24 * 60 * 60 * 1000).toISOString();
      ...
      if (isSupabaseConfigured && supabase) {
        try {
          if (productData.sellerId) {
            ...
            // Check seller subscription status before permitting product upload
            ...
            if (pData) {
              ...
              const eligibility = checkProductUploadEligibility(sellerProf);
              if (!eligibility.allowed) {
                throw new Error(eligibility.reason || 'Cannot upload product: Your subscription is unpaid or expired.');
              }
            }
          }
          const { data, error } = await supabase
