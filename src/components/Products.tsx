...
function ProductListingCard({ 
  ...
  const handleContactAction = (e: React.MouseEvent) => {
    ...
    if (!subInfo.isPaid) {
      e.preventDefault();
      e.stopPropagation();
      onRequireSubscription?.('update your subscription for quick and easy communication');
    }
  };
          ...
          <div className="mt-3 pt-2.5 border-t border-gray-100 flex flex-col space-y-1 text-left px-1">
            ...
            <div className="flex items-center space-x-2">
              ...
              <a
                ...
                className="flex-1 flex items-center justify-center space-x-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200/60 rounded-xl py-2 px-2.5 text-xs font-bold transition duration-200"
              ...
              {product.sellerEmail && (
                <a
                  ...
                  onClick={handleContactAction}
                  className="flex items-center justify-center bg-red-50 hover:bg-red-100 text-red-700 border border-red-200/60 rounded-xl p-2 text-xs font-bold transition duration-200 shrink-0"
                  title="Send Email"
                >
              ...
              {(product.sellerPhone || product.sellerWhatsapp) && (
                <a
                  ...
                  onClick={handleContactAction}
                  className="flex items-center justify-center bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200/60 rounded-xl p-2 text-xs font-bold transition duration-200 shrink-0"
                  title="Call Seller"
                >
              ...
              {product.sellerFacebook && (
                <a
                  ...
                  rel="noopener noreferrer"
                  onClick={handleContactAction}
                  className="flex items-center justify-center bg-blue-50 hover:bg-blue-100 text-blue-800 border border-blue-200/60 rounded-xl p-2 text-xs font-bold transition duration-200 shrink-0"
...
export default function Products({ user, onNavigateTab, onUpdateProfile }: ProductsProps) {
  ...
  const triggerSubscriptionModal = (message?: string, title?: string, subtitle?: string) => {
    ...
    setSubModalMessage(message || 'update your subscription to upload and sell more');
  ...
  const handleOpenAdd = () => {
    ...
    if (!eligibility.allowed) {
      triggerSubscriptionModal('update your subscription to upload and sell more', 'Subscription Required');
      return;
    }
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    if (!editingProduct && !eligibility.allowed) {
      setFormError('update your subscription to upload and sell more');
      triggerSubscriptionModal('update your subscription to upload and sell more', 'Subscription Required');
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-5">
        ...
        {user.role === 'seller' && (
          uploadEligibility.allowed ? (
            ...
            <button
              id="btn-add-harvest"
              onClick={() => triggerSubscriptionModal('update your subscription to upload and sell more', 'Subscription Required')}
              className="inline-flex items-center bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 px-5 py-3 rounded-xl text-xs font-bold shadow-xs transition cursor-pointer shrink-0"
              title="Subscription required to upload products"
            >
              <Lock className="h-4 w-4 mr-1.5 text-amber-700" />
              <span>Add Product (Subscription Required)</span>
            </button>
          )
        )}
      </div>
          ...
          <div className="flex items-start space-x-3.5">
            ...
            <div>
              <h4 className="text-sm font-bold text-amber-950 flex items-center gap-1.5">
                <span>Product Uploading Restricted — Subscription Required</span>
              </h4>
