...
import { LogisticsJob, UserProfile, getUserSubscriptionInfo } from '../types';
import { db } from '../lib/supabase';
import SubscriptionModal from './SubscriptionModal';
import { 
  Truck, ArrowRight, MapPin, Calendar, DollarSign, Package, 
  CheckCircle2, Clock, ShieldAlert, Navigation, User, X,
  Globe, Star, Sliders, Coins, Scale
} from 'lucide-react';
import { WhatsAppLogo, CallLogo, EmailLogo, FacebookLogo } from './BrandIcons';

// Helper to format registered logistics carriers
const formatCarrierDetails = (p: any) => {
  return {
    id: p.id,
    name: p.name || p.businessName || p.fullName,
    initial: p.initial || (p.businessName ? p.businessName.slice(0, 4).toUpperCase() : p.fullName?.slice(0, 4).toUpperCase() || 'LOG'),
    color: p.color || 'from-amber-600 to-amber-800',
    textColor: 'text-white',
    type: p.type || 'Registered Freight Provider',
    country: p.country || p.nationality || 'Malawi',
    phone: p.phone || '',
    whatsapp: p.whatsapp || p.phone || '',
    email: p.contactEmail || p.email || '',
    website: p.website || p.facebook || '',
    isGlobal: p.isGlobal || false,
    rating: p.rating || 5.0,
    baseRate: p.baseRate || 180,
    perTonRate: p.perTonRate || 30,
    transitDays: p.transitDays || '2-4 days',
    services: p.services || ['Agri-Bulk Haulage', 'Corridor Express Delivery', 'Direct Depot Dispatch'],
    logoUrl: p.logoUrl
  };
};
...
export default function Logistics({ user, onNavigateTab, onUpdateProfile }: LogisticsProps) {
  ...
  const triggerSubscriptionModal = (message?: string, title?: string, subtitle?: string) => {
    setSubModalTitle(title || 'Subscription Required');
    setSubModalMessage(message || 'update your subscription for quick and easy communication');
  ...
  const handleContactAction = (e: React.MouseEvent) => {
    ...
    if (!subInfo.isPaid) {
      e.preventDefault();
      e.stopPropagation();
      triggerSubscriptionModal('update your subscription for quick and easy communication', 'Subscription Required');
    }
  };
  ...
  const getMergedProviders = (country: string) => {
    ...
    return {
      domestic: domesticRegistered,
      international: otherRegistered,
      all: registeredCarriers
    };
  };
    ...
    try {
      ...
      const mappedRegisteredCarriers = providerProfiles.map(p => {
        ...
        return {
          ...
          color: color,
          textColor: 'text-white',
          type: 'Registered Freight Provider',
          ...
          services: ['Agri-Bulk Haulage', 'Corridor Express Dispatch', 'Direct Freight Delivery'],
          logoUrl: p.logoUrl
        };
      });
      ...
      {user.role === 'logistics_provider' ? (
        ...
        <div className="space-y-6">
                        ...
                        {job.providerId && (
                          <div className="flex items-center space-x-2 mt-1 flex-wrap gap-y-1">
                            ...
                            <a
                              ...
                              className="flex items-center space-x-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200/50 rounded-lg px-2.5 py-1 text-[11px] font-bold transition"
                            ...
                            {job.providerEmail && (
                              <a
                                ...
                                onClick={handleContactAction}
                                className="flex items-center space-x-1.5 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200/50 rounded-lg px-2.5 py-1 text-[11px] font-bold transition"
                                title="Email Shipper"
                              >
                                <EmailLogo className="h-3.5 w-3.5 shrink-0" />
                                <span>Email</span>
                              </a>
                            )}
                            ...
                            {(job.providerPhone || job.providerWhatsapp) && (
                              <a
                                ...
                                title="Call Shipper"
                              >
                                <CallLogo className="h-3.5 w-3.5 shrink-0" />
                                <span>Call</span>
                              </a>
                            )}
                            ...
                            {job.providerFacebook && (
                              <a
                                ...
                                rel="noopener noreferrer"
                                onClick={handleContactAction}
                                className="flex items-center space-x-1.5 bg-blue-50 hover:bg-blue-100 text-blue-800 border border-blue-200/50 rounded-lg px-2.5 py-1 text-[11px] font-bold transition"
                                title="Facebook Shipper"
                              >
                                <FacebookLogo className="h-3.5 w-3.5 shrink-0" />
                                <span>Facebook</span>
                              </a>
                            )}
                          </div>
          ...
          <div className="mt-12 border-t border-gray-100 pt-8 text-left">
            ...
            {/* Registered Carriers Directory */}
            {registeredCarriers.length === 0 ? (
              <div className="bg-white/60 backdrop-blur-md rounded-3xl border border-gray-100 p-8 text-center shadow-xs">
                <div className="w-12 h-12 bg-amber-50 rounded-2xl flex items-center justify-center mx-auto mb-3 text-amber-600">
                  <Truck className="h-6 w-6" />
                </div>
                <h4 className="text-sm font-sans font-black text-gray-800">No Registered Logistics Providers Yet</h4>
                <p className="text-xs text-gray-500 max-w-md mx-auto mt-1 leading-relaxed">
                  Verified transport and freight forwarders registered on AFKET will appear here for direct route booking and carriage rate estimates.
                </p>
              </div>
            ) : registeredCarriers.length <= 4 ? (
                ...
                {registeredCarriers.map((carrier) => {
                  ...
                  return (
                    <button
                      ...
                      className="group bg-white rounded-2xl p-4 border border-gray-100 hover:border-amber-400 hover:shadow-md transition duration-300 flex flex-col items-center text-center cursor-pointer"
                    >
                      <div className="w-16 h-16 rounded-full border-2 border-white shadow-md overflow-hidden relative transition-all duration-300 group-hover:scale-105 bg-white flex items-center justify-center shrink-0">
                        {carrier.logoUrl ? (
                          ...
                          <div className={`w-full h-full bg-gradient-to-br ${carrier.color} flex items-center justify-center`}>
                            <span className={`text-base font-black tracking-tight ${carrier.textColor}`}>
                              {carrier.initial}
                            </span>
                          </div>
                        )}
                        <span className="absolute bottom-0 right-0 bg-white text-[9px] font-black px-1.5 py-0.5 rounded-full text-gray-800 border border-gray-100 shadow-xs z-10">
                          {getFlagEmoji(carrier.country)}
                        </span>
                      </div>
                      <span className="text-xs font-black text-gray-800 mt-2.5 group-hover:text-amber-600 transition truncate w-full">
                        {carrier.name}
                      </span>
                      <span className="text-[9px] text-gray-400 font-bold uppercase tracking-wider mt-0.5">
                        {carrier.type}
                      </span>
                      <span className="mt-2 text-[10px] text-[#D97706] font-bold bg-amber-50 px-2 py-0.5 rounded-full">
                        View Rates & Contact
                      </span>
                    </button>
                  );
                ...
                <div className="border-t border-gray-100 mt-6 pt-5 space-y-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <a
                      ...
                      onClick={handleContactAction}
                      className="flex-1 bg-red-50 hover:bg-red-100 text-red-800 border border-red-100 rounded-xl py-3 text-xs font-bold transition flex items-center justify-center space-x-2"
                    >
                      <EmailLogo className="h-4 w-4 shrink-0" />
                    ...
                    <a
                      ...
                      className="flex-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-100 rounded-xl py-3 text-xs font-bold transition flex items-center justify-center space-x-2"
                    >
                      <CallLogo className="h-4 w-4 shrink-0" />
