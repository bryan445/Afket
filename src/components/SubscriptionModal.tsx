    ...
    try {
      ...
      const paychanguRes = await triggerPayChanguPayment({
        ...
        customization: {
          ...
          description: `PayChangu Payment for ${currentAmount.toLocaleString()} MWK ${planTitle}`
        }
      });
      ...
      setSuccessMsg(`Subscription activated! ${selectedPlan === 'annual' ? 'Annual' : 'Monthly'} access granted via PayChangu (Ref: ${finalRef}). You can now upload products.`);
      if (onSuccess) {
        onSuccess(updated);
      }
      setTimeout(() => {
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-lg rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-2xl text-left relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-amber-50/80 border border-amber-200/80 rounded-2xl p-3.5 mb-5 text-xs text-amber-900">
          ...
          <p className="leading-relaxed font-medium">
            {customMessage
              ? customMessage
              : subInfo.isDue
              ? `Your 14-day free trial or previous subscription expired on ${subInfo.formattedDueDate}. Please renew your subscription to continue trade operations.`
        ...
        <form onSubmit={handleProcessPayment} className="space-y-4">
          ...
          <div>
            <label className="text-xs font-bold text-gray-700 block mb-2">PayChangu Payment Channel (Malawi)</label>
            ...
            >
              {isProcessing ? (
                <>
                  ...
                  <span>Connecting PayChangu...</span>
                </>
              ) : (
                <>
